...
async function refreshUpdateBanner() {
  try {
    ...
    if (info && info.hasUpdate && !info.snoozed) {
      ...
      $('updateBanner').querySelector('.update-text').firstChild.textContent = info.forceUpdate
        ? 'Required update — v'
        : 'Update available — v';
...
$('test').addEventListener('click', async () => {
  const s = await chrome.storage.sync.get({ soundEnabled: true });
  chrome.runtime.sendMessage({ type: 'TEST_POPUP', city: 'Test City', sound: s.soundEnabled });
  window.close();
});
